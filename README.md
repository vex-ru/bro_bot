...
1) Сделать низкоуровневую прошивку для движения моторов VexV5 на Mega2560
2) Создать прошивку ESP32 - MicroROS
3) Сделать интеграцию между Mega2560 и ESP32
4) Сделать интеграцию между ESP32 и Ubuntu 20.04
5) Создать цифрового двойника в RVIZ
6) Решить задачу одометрии - если я кручу колеса на физическом роботе, то они крутятся на виртуальном и наоборот
7) Решить задачу одометрии учитывая параметры колес и размера робота, сделать эксперименты по движению на определенное расстояние, угол поворота

## Манипулятор
После решения задачи одометрии тележки

## Камера
После решения задачи одометрии манипулятора
1) Распознавание объектов, подключение PhisicalAI, Aruco Маркеры, YOLO распознавание и обучение на мусоре
2) Интеграция камеры и Манипулятора (камера на манипуляторе) для точного позиционирования